...
class StackFrameManager {
   private:
    ...
    std::unordered_map<unsigned, int> localVarToStackSlot;
    std::unordered_map<const midend::Value*, int> allocaToStackSlot;
    std::unordered_set<unsigned> calleeSavedRegs;
   ...
   private:
    ...
    // 指令生成辅助函数
    void insertInstructionAtBeginning(BasicBlock* bb,
                                      std::unique_ptr<Instruction> inst);
    void insertInstructionAtEnd(BasicBlock* bb,
                                std::unique_ptr<Instruction> inst);
    std::unique_ptr<Instruction> createStackAdjustInstruction(int offset);
    std::unique_ptr<Instruction> createSaveInstruction(unsigned regNum,
                                                       int offset);
    std::unique_ptr<Instruction> createRestoreInstruction(unsigned regNum,
                                                          int offset);
};
