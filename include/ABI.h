...
// 根据ABI名称获取寄存器编号
    ...
    bool isCallerSaved(unsigned physreg, bool isFloat = false);
    
    // 判断是否为被调用者保存寄存器 (Callee-saved/Saved registers)
    bool isCalleeSaved(unsigned physreg, bool isFloat = false);
    
    // 判断是否为参数寄存器 (Argument registers)
    bool isArgumentReg(unsigned physreg, bool isFloat = false);
    
    // 判断是否为返回值寄存器 (Return value registers)
    bool isReturnReg(unsigned physreg, bool isFloat = false);

    bool isReservedReg(unsigned physreg, bool isFloat = false);
