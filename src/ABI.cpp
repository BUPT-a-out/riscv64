...
// 整数寄存器ABI名称映射
static const std::unordered_map<std::string, unsigned>&
getIntAbiNameToNumMap() {
...
// 浮点寄存器ABI名称映射
static const std::unordered_map<std::string, unsigned>&
getFloatAbiNameToNumMap() {
    static const std::unordered_map<std::string, unsigned> floatNameMap = {
        {"ft0", 0},   {"f0", 0},   {"ft1", 1},   {"f1", 1},   {"ft2", 2},
        {"f2", 2},    {"ft3", 3},  {"f3", 3},    {"ft4", 4},  {"f4", 4},
        {"ft5", 5},   {"f5", 5},   {"ft6", 6},   {"f6", 6},   {"ft7", 7},
        {"f7", 7},    {"fs0", 8},  {"f8", 8},    {"fs1", 9},  {"f9", 9},
        {"fa0", 10},  {"f10", 10}, {"fa1", 11},  {"f11", 11}, {"fa2", 12},
        {"f12", 12},  {"fa3", 13}, {"f13", 13},  {"fa4", 14}, {"f14", 14},
        {"fa5", 15},  {"f15", 15}, {"fa6", 16},  {"f16", 16}, {"fa7", 17},
        {"f17", 17},  {"fs2", 18}, {"f18", 18},  {"fs3", 19}, {"f19", 19},
        {"fs4", 20},  {"f20", 20}, {"fs5", 21},  {"f21", 21}, {"fs6", 22},
        {"f22", 22},  {"fs7", 23}, {"f23", 23},  {"fs8", 24}, {"f24", 24},
        {"fs9", 25},  {"f25", 25}, {"fs10", 26}, {"f26", 26}, {"fs11", 27},
        {"f27", 27},  {"ft8", 28}, {"f28", 28},  {"ft9", 29}, {"f29", 29},
...
unsigned getRegNumFromABIName(const std::string& name, bool isFloat) {
    const auto& map =
        isFloat ? getFloatAbiNameToNumMap() : getIntAbiNameToNumMap();
...
bool isCallerSaved(unsigned physreg, bool isFloat) {
    if (isFloat) {
        // 浮点Caller-saved寄存器: ft0-ft7 (0-7), ft8-ft11 (28-31), fa0-fa7
        // (10-17)
        return (physreg >= 0 && physreg <= 7) ||    // ft0-ft7
               (physreg >= 10 && physreg <= 17) ||  // fa0-fa7
               (physreg >= 28 && physreg <= 31);    // ft8-ft11
    } else {
        // 整数Caller-saved寄存器: t0-t2 (5-7), t3-t6 (28-31), a0-a7 (10-17), ra
        // (1)
